...
if __name__ == "__main__":
    ...
    with open('noized_triangle_metaoptimize_{}.pickle'.format(1), 'rb') as f:
    ...
    global_sum_error = np.array(len(errors), )
    for i in range(1, 9):
        print("NOW IS STAGE {}".format(i))

        with open('noized_triangle_metaoptimize_{}.pickle'.format(i), 'rb') as f:
            loaded = pickle.load(f)
        ...
        for j in range(len(errors)):
            print(j)
            try:
                sum_error.append(np.sum(errors[j][:]))
                # -100 is hardcoded constant to know that in this point algorithm not works
            except Exception:
                if (errors[j] == np.inf):
                    sum_error.append(900000)
                pass
        sum_error = np.array(sum_error)
        global_sum_error = global_sum_error+sum_error


    np_sum_error = np.array(global_sum_error)
    # print(np_sum_error)

    for i in range(len(np_sum_error)):
        if(np_sum_error[i]<9000000):
            print("Good parameters is beta : {}, gamma :{}".format(B_[i], G_[i]))
